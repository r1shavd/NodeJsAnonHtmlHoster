...
uploadBtn.addEventListener('click' ,(e) => {
	...
	if (/[a-zA-Z0-9]{5,30}/.test(fileIdInput.value) && fileIdInput.value.includes(' ') == false) {
		...
		fetch('/', {body : data, method : 'post',  headers : {'Accept': 'application/json', 'Content-Type' : 'application/json'}}).then(response => response.text()).then(response => {
			...
			if (response == 'true') {
				...
				if (htmlDataInput.value.length > 10) {
					...
					fetch('/', {body : data, method : 'post',  headers : {'Accept': 'application/json', 'Content-Type' : 'application/json'}}).then(response => response.text()).then(response => {
						...
						// If there are errors in the process, then we display the error back to the user using the formErrorText field
			...
			/* If there are any errors in the process, then we write it in the formErrorText */
		...
		formErrorText.innerText = 'Error : Please choose a proper alphanumeric file Id ranging from 5 to 30 characters.';
		formErrorText.style.display = 'block';
	}
});